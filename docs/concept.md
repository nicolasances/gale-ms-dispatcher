...
| OQ-05 | ~~`taskId` format?~~ **Decided: `crypto.randomUUID()`.** | Implemented in #1. No dependency, and `createdAt` carries the sort order since a UUID has none. A ULID-style prefix would have made ids sortable and greppable in GCS listings; that remains the change to make if listing tasks by id ever becomes a real workflow, and it is a breaking change for ids already minted. |
